...
`download` 在内容变化时先于实际文件旁创建 `config.pdo-backup-<UTC时间戳>`，再替换文件。若 `~/.ssh/config` 是符号链接，会保留链接并更新最终目标；悬空链接、循环链接及非普通文件会被拒绝。
...
推送 `v*` tag 会在测试通过后发布 macOS、Linux、Windows 的 `amd64` / `arm64` 二进制、安装脚本及 `SHA256SUMS`。
